...
3. Use its buttons to exercise apply damage with c#, apply damage with component, disable / enable actor.
4. Inspect the configured hosts and triggers, then open
   [CombatWorkflow.cs](../Samples~/DefinitionWorkflow/Runtime/CombatWorkflow.cs). It is the caller
...
- **Apply damage with C#**: `CombatWorkflow.Damage()`.
- **Apply damage with component**: `CombatWorkflow.DamageComponent()`.
- **Disable / enable actor**: `CombatWorkflow.Reactivate()`.
